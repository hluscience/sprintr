...
sis_lasso <- function(x, y, family = "gaussian", num_keep = NULL, ...){
  ...
  if(inherits(x, "sparseMatrix")){
    idx <- packa::screen_sparse_cpp(x = x, y = y, nLev = 1, offset = offset,  num_keep = num_keep, square = FALSE, main_effect = FALSE, family = family)
  }else{
    idx <- packa::screen_cpp(x = x, y = y, nLev = 1, offset = offset, num_keep = num_keep, square = FALSE, main_effect = FALSE,  family = family)
  }
