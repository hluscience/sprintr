...
sprinter <- function(x, y, family = "gaussian", nlev = 1, square = FALSE, num_keep = NULL, lambda1 = NULL, lambda3 = NULL, cv_step1 = FALSE, nlam1 = 10, nlam3 = 100, lam_min_ratio = ifelse(nrow(x) < ncol(x), 0.01, 1e-04), ...){
  ...
  if (ncol(step1$beta) < nlam1) {
    # This could happen when we use cv.sprinter to call sprinter to conduct cross-validation to select the best lambda
    # it only happens when glmnet returns a numerical warning as follows:
    ...
    # when this happens, not all nlam1 lambda values in Step1 is returned
    # we just copy the prediction from the last viable lambda values to fill in the empty slots
    last_col <- ncol(step1$beta)
    ncol_to_add <- nlam1 - last_col

    # Copy the last column values to fill in the empty slots
    step1$beta <- cbind(step1$beta, matrix(rep(step1$beta[, last_col], ncol_to_add), ncol = ncol_to_add))
    step1$a0 <- c(step1$a0, rep(step1$a0[last_col], ncol_to_add))
    step1$mu <- cbind(step1$mu, matrix(rep(step1$mu[, last_col], ncol_to_add), ncol = ncol_to_add))
  }
