...
#' @import glmnet
#' @export
apl <- function(x, y, eta = 1, family = "gaussian", type.measure = "deviance", ...){
  
  # x is the unstandardized design matrix
  # eta controls the relative penalty between main effects and interactions
  # through penalty.factor option in glmnet
  p <- ncol(x)
  n <- nrow(x)
  q <- (p^2 + p) / 2
  ...
  fit <- glmnet::cv.glmnet(x = design, y = y, 
                           # penalty.factor is used to control the relative penalty in main effects and interactions
                           # eta <= 1 allows interactions to be penalized more harshly than main effects
                           penalty.factor = c(rep(eta, p), rep(1, q)),
                           family = family,
