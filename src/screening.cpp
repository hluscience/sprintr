...
//' @param family Either a character string representing one of the built-in exponential families, including "gaussian", "poisson", "binomial", "multinomial", "ordinal". Default is "gaussian".
...
// Below implementation modifies the Sure Independence Screening (SIS) step for the binomial family
// by replacing the Marginal Maximum Likelihood (MML) estimator with the Maximum Marginal Likelihood
// Estimator (MMLE).

// Newton-Raphson for logistic MMLE
// [[Rcpp::export]]
double logistic_MMLE(const arma::vec & x, const arma::vec & y, const arma::vec & offset){
...
private:
  // j, k are the indices of the interaction
  int j, k;
  // the MMLE of the j,k interaction with some target
  double MMLE;
public:
  MMLE_Triplet(int _j, int _k, double _MMLE)
  {
    j = _j;
    k = _k;
    MMLE = _MMLE;
  ...
  double get_MMLE() const{
    return MMLE;
  }
};

// user specified comparator for MMLE
class MMLE_Triplet_Comparator{
public:
  bool operator() (const MMLE_Triplet & p1, const MMLE_Triplet & p2) const {
    // note that here we use > instead of <
    // so that the resulting heap is a min-heap
    // (by default the C++ priority_queue is a max-heap)
    return p1.get_MMLE() > p2.get_MMLE();
  }
};
...
//' @param square An indicator of whether squared effects should be considered in Step 1 (NOT Step 2!). square == TRUE if squared effects have been considered in Step 1, i.e., squared effects will NOT be considered in Step 2.
//' @param main_effect An indicator of whether main effects should also be screened. Default to be false.
//' @return a three-column matrix representing the index pair of the selected interactions and its MMLE with the response vector given by the \code{offset}.
//' @export
// [[Rcpp::export]]
arma::mat screen_cpp_MMLE_binomial(const arma::mat & x, const arma::vec & y, const arma::mat & offset, const int & num_keep, const bool square = false, const bool main_effect = false){
   // main is TRUE if we want to screen main effects simultaneously
   // square is TRUE if squared effects have been considered in the first step
   // i.e., squared effects will NOT be considered in the screening step!
   // screening all the interaction x_j*x_k based on its MMLE
   // return the indices (j, k) of the top k interactions
   ...
   // Creates a Min heap of Triplets (ordered by MMLE)
   std::priority_queue <MMLE_Triplet, std::vector<MMLE_Triplet>, MMLE_Triplet_Comparator> min_heap;
   ...
   if(square){
     for(j = 0; j < p-1; ++j){
       for (k = j+1; k < p; ++k){
         temp_vec = x.col(j) % x.col(k);
         temp_vec = (temp_vec - arma::mean(temp_vec))/arma::stddev(temp_vec);
         cur_MMLE = logistic_MMLE(temp_vec, y, offset);
         if (count < num_keep){
           // use emplace instead of push
           // emplace avoids constructing a temporary Triplet
           // emplace requires C++11, to use it, add
           // // [[Rcpp::plugins(cpp11)]] in the top of the file
           // min_heap.emplace(j, k, cur_MMLE);
           min_heap.push(MMLE_Triplet(j, k, cur_MMLE));
           ++count;
         } else{
           if (cur_MMLE > min_heap.top().get_MMLE()){
             // pop the current top element
             min_heap.pop();
             // insert the (j,k) interaction
             //min_heap.emplace(j, k, cur_MMLE);
   ...
   if(main_effect){
     for(k = 0; k < p; ++k){
       temp_vec = x.col(k);
       temp_vec = (temp_vec - arma::mean(temp_vec))/arma::stddev(temp_vec);
       cur_MMLE = logistic_MMLE(temp_vec, y, arma::zeros(n));
       if (cur_MMLE > min_heap.top().get_MMLE()){
         // pop the current top element
         min_heap.pop();
         // insert the (j,k) interaction
         // for main effects, it corresponds to (0, k)
         min_heap.push(MMLE_Triplet(-1, k, cur_MMLE));
       }
     }
   }
   ...
   for (count = 0; count < num_keep; ++count){
     result(count, 0) = min_heap.top().get_j() + 1;
     result(count, 1) = min_heap.top().get_k() + 1;
     result(count, 2) = min_heap.top().get_MMLE();
     min_heap.pop();
   }
   return result;
}
